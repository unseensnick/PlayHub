...
const INVADER_SPEED = 0.6; // Decreased by 40%
...
function createBarriers() {
    ...
    const barrierWidth = 100; // Increased from 80
    const barrierHeight = 60;
    const spacing = 60; // Increased for better spacing
    const startX = (CANVAS_WIDTH - (barrierCount * barrierWidth + (barrierCount - 1) * spacing)) / 2;
    ...
    switch (action.type) {
        ...
        case 'LEVEL_COMPLETE':
            return {
                ...
                gameHistory: [...state.gameHistory, { type: "level_complete", level: state.level }]
            };
...
function updateGameLogic(state, keys) {
    ...
    if ((keys.ArrowRight || keys.KeyD) && state.player.x < CANVAS_WIDTH - PLAYER_WIDTH) {
        newState.player = { ...state.player, x: state.player.x + PLAYER_SPEED };
    }
    ...
    newState.gameHistory = [...newState.gameHistory, ...invaderBulletUpdates.events];
...
function updateBullets(bullets, state) {
    ...
    return { bullets: newBullets, invaders, barriers, powerUps: newPowerUps, newScore, newEvents };
}

function updateInvaders(state) {
    if (state.invaders.length === 0) return { invaders: state.invaders, direction: state.invaderDirection };
    
    let shouldMoveDown = false;
    let newDirection = state.invaderDirection;
    
    const leftmostX = Math.min(...state.invaders.map(inv => inv.x));
    const rightmostX = Math.max(...state.invaders.map(inv => inv.x + inv.width));
    ...
    const newInvaders = state.invaders.map(invader => ({
        ...invader,
        x: shouldMoveDown ? invader.x : invader.x + state.invaderSpeed * state.invaderDirection,
...
function updateInvaderBullets(state) {
    ...
    if (newLastShot >= newCooldown && state.invaders.length > 0) {
        const frontInvaders = state.invaders.filter(invader => {
            const sameColumn = state.invaders.filter(other => 
    ...
    return { bullets: newBullets, lastShot: newLastShot, cooldown: newCooldown, lives: newLives, barriers, events };
}
            ...
            if (powerUp.type === 'life') {
                ...
                for (let j = -1; j <= 1; j++) {
                    newBullets.push({
                        ...
                        x: state.player.x + PLAYER_WIDTH / 2 - BULLET_WIDTH / 2 + j * 15,
                        y: state.player.y,
                        width: BULLET_WIDTH,
                        height: BULLET_HEIGHT,
                        dy: -BULLET_SPEED
...
export function SpaceInvaders() {
    ...
    useEffect(() => {
        if (gameState.gameState === GAME_STATES.PLAYING) {
            ...
            // Check lose conditions
                     ...
                     gameState.invaders.some(invader => invader.y + invader.height >= gameState.player.y)) {
                dispatch({ type: 'GAME_OVER' });
            }
        }
    }, [gameState.invaders.length, gameState.lives, gameState.gameState, gameState.invaders, gameState.player.y]);
    ...
    const draw = useCallback(() => {
        ...
        for (let i = 0; i < 50; i++) {
            ...
            const y = (i * 456 + gameState.animationFrame * 0.5) % CANVAS_HEIGHT;
        ...
        if (gameState.gameState === GAME_STATES.LEVEL_COMPLETE) {
            ...
            ctx.font = "24px Arial";
            ctx.fillText(`Advancing to Level ${gameState.level + 1}`, CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 50);
        }
    }, [gameState]);
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="flex justify-center gap-4">
                        {gameState.gameState === GAME_STATES.MENU && (
                            <Button onClick={() => dispatch({ type: 'START_GAME' })} size="lg" className="gap-2">
                        ...
                        {gameState.gameState === GAME_STATES.PLAYING && (
                            <Button onClick={() => dispatch({ type: 'PAUSE_GAME' })} variant="outline" size="lg" className="gap-2">
                        ...
                        {gameState.gameState === GAME_STATES.PAUSED && (
                            <>
                                <Button onClick={() => dispatch({ type: 'RESUME_GAME' })} size="lg" className="gap-2">
                                    <Play className="h-4 w-4" />
                                    Resume
                                </Button>
                                <Button onClick={() => dispatch({ type: 'RESET_GAME' })} variant="outline" size="lg" className="gap-2">
                        ...
                        {gameState.gameState === GAME_STATES.GAME_OVER && (
                            <Button onClick={() => dispatch({ type: 'RESET_GAME' })} size="lg" className="gap-2">
                                <RotateCcw className="h-4 w-4" />
                                Play Again
                            </Button>
                        )}
                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        <span className="font-medium">
                                            {event.type === "invader_destroyed" && `Invader destroyed! +${event.points} points`}
                                            {event.type === "player_hit" && `Hit! ${event.livesRemaining} lives remaining`}
                                            {event.type === "level_complete" && `Level ${event.level} complete!`}
                                            {event.type === "life_gained" && "Extra life gained!"}
                                            {event.type === "multishot_gained" && "Multishot power-up!"}
                                        </span>
                                        {event.score && (
