...
export function Pong() {
    ...
    const gameObjects = useRef({
        ...
        aiPaddle: { x: CANVAS_WIDTH - 30 - PADDLE_WIDTH, y: CANVAS_HEIGHT / 2 - PADDLE_HEIGHT / 2 },
        ball: { 
            x: CANVAS_WIDTH / 2, 
            y: CANVAS_HEIGHT / 2, 
    ...
    const aiState = useRef({
        ...
        reactionDelay: 0,
        maxReactionDelay: 25, // Balanced delay
        lastBallDirection: 1,
        errorOffset: 0,
        updateCounter: 0,
        rallyCount: 0,
        lastBallSpeed: BALL_SPEED,
        confidenceLevel: 0.4, // Start with low confidence
        lastMissTime: 0,
        consecutiveHits: 0
    });
    ...
    const resetGame = useCallback(() => {
        ...
        setAiScore(0);
        setGameHistory([]);
        gameObjects.current.playerPaddle.y = CANVAS_HEIGHT / 2 - PADDLE_HEIGHT / 2;
    ...
    const updateGame = useCallback(() => {
        ...
        if ((keysRef.current.ArrowUp || keysRef.current.KeyW) && playerPaddle.y > 0) {
            playerPaddle.y -= PADDLE_SPEED;
        }
        if ((keysRef.current.ArrowDown || keysRef.current.KeyS) && playerPaddle.y < CANVAS_HEIGHT - PADDLE_HEIGHT) {
            playerPaddle.y += PADDLE_SPEED;
        }
        ...
        const currentBallSpeed = Math.sqrt(ball.dx * ball.dx + ball.dy * ball.dy);
        ...
        if (currentBallSpeed > aiData.lastBallSpeed * 1.1) {
            aiData.confidenceLevel = Math.max(0.1, aiData.confidenceLevel - 0.25);
        } else if (aiData.consecutiveHits > 5) {
            aiData.confidenceLevel = Math.min(0.6, aiData.confidenceLevel + 0.02); // Cap at 0.6
        }
        ...
        const adaptiveMaxDelay = Math.floor(25 + (speedMultiplier - 1) * 15 + (1 - aiData.confidenceLevel) * 12);
        
        aiData.updateCounter++;
        if (aiData.updateCounter % Math.max(4, Math.floor(6 - speedMultiplier)) === 0) { // More frequent updates
            const currentBallDirection = ball.dx > 0 ? 1 : -1;
            
            if (currentBallDirection !== aiData.lastBallDirection && currentBallDirection > 0) {
                ...
                const confidenceAdjustedError = baseError * (2.0 - aiData.confidenceLevel); // Less error scaling
                aiData.errorOffset = (Math.random() - 0.5) * confidenceAdjustedError;
            }
            ...
            if (aiData.reactionDelay <= 0 && ball.dx > 0) {
                ...
                const timeToReachPaddle = (aiPaddle.x - ball.x) / Math.abs(ball.dx);
                ...
                if (predictedY < 0 || predictedY > CANVAS_HEIGHT) {
                    const bounceY = predictedY < 0 ? -predictedY : 2 * CANVAS_HEIGHT - predictedY;
                    predictedY = bounceY;
                }
                
                // Moderate prediction errors
                const predictionAccuracy = aiData.confidenceLevel * (0.7 / speedMultiplier); // Better accuracy
                const predictionError = (Math.random() - 0.5) * 60 * (2.5 - predictionAccuracy); // Smaller errors
                
                aiData.targetY = predictedY + predictionError + aiData.errorOffset;
                ...
                aiData.targetY = Math.max(PADDLE_HEIGHT / 2, 
                ...
                const variation = (Math.random() - 0.5) * (45 + aiData.rallyCount * 3); // Less wandering
        ...
        if (Math.abs(targetDiff) > adaptiveDeadZone) {
            ...
            const urgencyMultiplier = Math.min(1.6, Math.abs(targetDiff) / 60) * cornerUrgency;
            
            const movementSpeed = baseAiSpeed * urgencyMultiplier;
            const speedVariation = (Math.random() - 0.5) * 0.8 * (2.5 - aiData.confidenceLevel); // Less variation
            
            const finalSpeed = Math.max(0.6, movementSpeed + speedVariation); // Faster minimum
            const newY = aiPaddle.y + (moveDirection * finalSpeed);
            aiPaddle.y = Math.max(0, Math.min(CANVAS_HEIGHT - PADDLE_HEIGHT, newY));
        }
        ...
        if (ball.x - BALL_SIZE / 2 <= playerPaddle.x + PADDLE_WIDTH &&
        ...
        if (ball.x < 0) {
            ...
            setGameHistory(history => [...history, { type: "ai_score", score: `${playerScore}-${newAiScore}` }]);
            // Reset AI performance tracking
            aiState.current.consecutiveHits = 0;
            aiState.current.rallyCount = 0;
            aiState.current.confidenceLevel = Math.min(0.6, aiState.current.confidenceLevel + 0.05); // Cap at 0.6
            ...
            setGameHistory(history => [...history, { type: "player_score", score: `${newPlayerScore}-${aiScore}` }]);
            // AI missed - decrease confidence and reset tracking
            aiState.current.consecutiveHits = 0;
            aiState.current.rallyCount = 0;
            aiState.current.confidenceLevel = Math.max(0.1, aiState.current.confidenceLevel - 0.3); // Bigger penalty
                    ...
                    {gameState === "gameOver" && (
                        <div className="text-center p-6 rounded-lg bg-muted/30">
                            <div className="text-2xl font-semibold mb-2">
                                {playerScore > aiScore ? "Congratulations! You won!" : "AI wins this time!"}
                            </div>
                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        <span className="font-medium">
                                            {event.type === "player_score" ? "You scored!" : "AI scored!"}
                                        </span>
